$(document).ready(function () {
        ...
        if(validator.form()) {
            ...
            $.ajax($form.attr('action'), {
                ...
                else
                {
                    ...
                    //Eliminar boton de adicionar y mensaje de no direccion, cuando estamos en la interfaz de conf envio publicar anuncio

                    $('#addDireccionCompra').hide();

                    $('#msg-no-direccion-compra').hide();

                    updateCostoEnvio(); //Funcion para actualizar el costo de envio segun la direccion que se agrego

                    // Se encuentra en confEnvioCesta.js


                }
                $('#btnNextConfEnvio').removeAttr('disabled');
    ...
    $('#provinciaEditar').on('change', function () {
        ...
        BuscarCiudades($(this).val(),"#containerDireccion","#ciudadEditar");
    ...
    $('#btnConfirmDanger-eliminarDireccion').on('click',function () {
        ...
        $.ajax(url, {
            ...
            alertify.success('Dirección eliminada satisfactoriamente.');
        });
            ...
            $.ajax($form.attr('action'), {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        tagDir='<div class="form-group" data-id="'+response.id+'">'+
                            '<label>'+$('#calleEditar').val()+' ('+$('#numeroEditar').val()+') '+'</label>'+
                            ' '+ciudad+', '+provincia+' - '+
                            '<span class="label label-default">Compra</span><br/><br/>'+
                            '<a id="m'+response.id+'" class="btn btn-primary btn-sm modificarDirec" href="#modalEditarDireccion" data-toggle="modal"><i class="fa fa-edit"></i> Modificar</a>'+
                        ...
                        $('#direccionCompra .form-group').each(function () {
                            if($(this).attr('data-id') == response.id){
                                $(this).remove();
                            }
                        });

                        //Adicionar direccion modificada
                ...
                alertify.success('La dirección ha sido editada satisfactoriamente.');
